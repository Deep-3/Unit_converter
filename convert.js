...
// 
// calculate given equation as per value of x and y
// 
function calculateEqu() {
  var xInput = document.getElementById('x');
  var yInput = document.getElementById('y');
  var equationInput = document.getElementById('equation').value.toLowerCase(); // Convert equation to lowercase
  var equationAns = document.getElementById('equation-ans');

  var x = parseFloat(xInput.value);
  var y = parseFloat(yInput.value);
  var equation = equationInput.value;

  equationAns.textContent = '';

  if (!isNaN(x) && equation.includes('y')) {
    yInput.disabled = true;
    yInput.value = '';

    // Replace x in the equation with the provided value
    var equationWithX = equation.replace(/x/g, x);
    var result = eval(equationWithX);

    equationAns.textContent = 'Y = ' + result;
  } else if (!isNaN(y) && equation.includes('x')) {
    xInput.disabled = true;
    xInput.value = '';

    // Replace y in the equation with the provided value
    var equationWithY = equation.replace(/y/g, y);
    var result = eval(equationWithY);

    equationAns.textContent = 'X = ' + result;
  } else {
    equationAns.textContent = 'Invalid input';
  }
}
